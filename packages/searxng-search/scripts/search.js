...
// Auto-detect Docker: use container hostname inside Docker, localhost otherwise
function getDefaultSearxngUrl() {
  const isDocker = fs.existsSync('/.dockerenv') || process.env.DOCKER_CONTAINER === 'true';
  return isDocker ? 'http://searxng:8080' : 'http://localhost:8080';
}

const SEARXNG_URL = process.env.SEARXNG_URL || getDefaultSearxngUrl();
